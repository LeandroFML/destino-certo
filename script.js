const hospedagens = [
  {
    ...
    preco: 280,
    imagem: "img/copacabana.jpg",
    descricao: "A 5 minutos da praia. Ideal para cadeirantes. Wi-Fi, ar-condicionado, cozinha equipada.",
    acessibilidade: ["Rampas de acesso", "Banheiro adaptado", "Elevador"],
    locaisProximos: [
      { nome: "Praia de Copacabana", acessivel: true },
      { nome: "Museu Histórico do Exército", acessivel: false }
    ]
  },
  {
    nome: "Flat Santa Teresa",
    preco: 320,
    imagem: "img/santateresa.jpg",
    descricao: "Vista panorâmica do Rio. Espaço moderno. Sem acessibilidade total.",
    acessibilidade: ["Entrada nivelada"],
    locaisProximos: [
      { nome: "Escadaria Selarón", acessivel: false },
      { nome: "Parque das Ruínas", acessivel: true }
    ]
  }
];
...
function atualizarAreaLogin() {
  ...
  if (area && nome) {
    area.innerHTML = `<p>👋 Olá, ${nome}! <button onclick="logout()">Sair</button></p>`;
  }
}
...
function renderizarHospedagens(lista) {
  const container = document.getElementById("listaHospedagens");
  if (!container) return;

  container.innerHTML = "";

  if (lista.length === 0) {
    container.innerHTML = "<p>Nenhuma hospedagem encontrada 😢</p>";
    return;
  }

  lista.forEach(h => {
    const locais = h.locaisProximos.map(l =>
      `<li>${l.acessivel ? "✅" : "❌"} ${l.nome}</li>`
    ).join("");

    const acess = h.acessibilidade.map(a =>
      `<li>✅ ${a}</li>`
    ).join("");
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        <h2>${h.nome}</h2>
        <p><strong>💰 R$${h.preco}</strong> - ${h.descricao.split(".")[0]}</p>
        <h3>♿ Acessibilidade:</h3>
        <ul>${acess}</ul>
        <h3>📍 Locais turísticos:</h3>
        <ul>${locais}</ul>
        <div class="btn-group">
          <button onclick="reservar('${h.nome}')">Reservar</button>
          <button onclick="favoritar('${h.nome}')">❤️ Favoritar</button>
        </div>
      </div>
    `;
...
function filtrarHospedagem() {
  const precoMax = parseFloat(document.getElementById("precoFiltro").value);
  const nomeBusca = document.getElementById("buscaNome").value.toLowerCase();

  const filtradas = hospedagens.filter(h => {
    const precoOk = isNaN(precoMax) || h.preco <= precoMax;
    const nomeOk = h.nome.toLowerCase().includes(nomeBusca);
    return precoOk && nomeOk;
  });

  renderizarHospedagens(filtradas);
}
...
function favoritar(nome) {
  ...
  if (!favoritos.includes(nome)) {
    ...
    alert(`❤️ ${nome} adicionado aos favoritos!`);
  } else {
    alert(`⚠️ ${nome} já está nos seus favoritos.`);
  }
}
...
function mostrarFavoritos() {
  ...
  const lista = hospedagens.filter(h => favoritos.includes(h.nome));
  renderizarHospedagens(lista);
}

function reservar(nome) {
  let reservas = JSON.parse(localStorage.getItem("reservas")) || [];
  reservas.push({ nome: nome, data: new Date().toLocaleDateString() });
  localStorage.setItem("reservas", JSON.stringify(reservas));
  alert(`✅ Reserva feita com sucesso para ${nome}`);
}

function mostrarReservas() {
  const reservas = JSON.parse(localStorage.getItem("reservas")) || [];
  const nomesReservados = reservas.map(r => r.nome);
  const lista = hospedagens.filter(h => nomesReservados.includes(h.nome));
  renderizarHospedagens(lista);
}

window.onload = () => {
  atualizarAreaLogin();
  if (window.location.pathname.includes("catalogo")) filtrarHospedagem();
};
